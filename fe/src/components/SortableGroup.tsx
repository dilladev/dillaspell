...
const SortableGroup: React.FC<SortableGroupProps> = ({
  ...
  const useAuth = import.meta.env.VITE_USE_ENTRA_AUTH === 'true';
  ...
  const saveSort = async (group: any) => {
    const newToken = await getToken();
  ...
  const deleteLink = useCallback(
    async (linkId: string) => {
      const newToken = await getToken();
      setShowConfirmationDialog({ isOpen: true, message: 'Are you sure you want to delete this link?', onConfirm: async () => {
        try {
        
          await fetch(`/links/${linkId}`, { method: 'DELETE', headers: {
            "Authorization": `Bearer ${newToken}`,
            "Content-Type": "application/json",
          }, })
